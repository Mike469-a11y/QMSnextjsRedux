...
import { useQmsData } from '../hooks/useQms';
...
const Qms = () => {
    ...
    const currentUser = useSelector((state) => state.user.currentUser.login);
    
    // TanStack Query for data fetching
    const {
        data: qmsData,
        isLoading: qmsLoading,
        error: qmsError,
        refetch: refetchQms
    } = useQmsData();
    ...
    if (qmsLoading) {
        return <Loading text="Loading QMS data..." />;
    }
    ...
    return (
        <div className="qms-container">
            {/* Demo: Show API data */}
            {qmsData && qmsData.length > 0 && (
                <div style={{ padding: '10px', background: '#f0f8ff', marginBottom: '20px', borderRadius: '4px' }}>
                    <strong>Demo API Data:</strong> Loaded {qmsData.length} items from TanStack Query
                </div>
            )}
